...
/**
 * the methods used to carry out communication between client and server through
 * the use of post and get requests,
 * the first are used to update values in the server, while the second is used
 * to obtain values from the server.
...
public interface Dashboard {
    ...
    /**
     * Retrieves all relevant information to update the Dashboard from the provided
     * instance of
     * shared memory.
     * 
     * @param shMemory the shared memory containing the actual data measured by the
     *                 physical sensors.
     */
    void refreshDashboardWithDataFromSharedMemory(final SharedMemory shMemory);

    /**
     * A method to check if a web application user requested a change of the opening
     * level of the valve.
     * 
     ...
     * doesn't necessarily imply that the valve is now opened at the requested
     * level,
